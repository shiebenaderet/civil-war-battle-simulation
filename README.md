...
- **v3.24.0** - Reflection before the quiz, with brainstorm helpers. Each act now ends with the reflection prompt first (so students write their handout answer), then the comprehension quiz as a check, instead of the reverse. The on-screen reflection question now matches each tier's handout question word-for-word. Two bright "Not sure what to write?" helper boxes scaffold the writing and scale by reading level: a sentence frame to finish at Most Support, a starter plus a question at More Support, brainstorm questions at Standard, and analytical questions at Extra Challenge. One helper is for the reflection answer, one for the "how this act helped the Union win" line. Extra Challenge helpers add a "Stuck?" thinking-move (take a position, back it with one specific fact) so advanced students get support too. Academic terms (Pyrrhic victory, Reconstruction, Clausewitz, and others) and the battle names used in the helpers are click-to-define in the glossary, and the hardest terms appear only at the Extra Challenge tier.
